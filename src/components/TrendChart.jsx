...
export default function TrendChart() {
  const data = useMemo(() => genTrend(), []);
  const W = 900, H = 220, pad = 30;
  const max = 80, min = 15;

  const threshY = pad + (1 - (32 - min) / (max - min)) * (H - pad * 2);
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="trend-svg">
      ...
      {days.map((d, i) => {
        ...
        return (
          <text key={d} x={x} y={H - 8} fill="#6E6353" fontFamily="IBM Plex Mono" fontSize="10" textAnchor="middle">
            {d}
          </text>
        );
      })}
